...
if(empty($result)){
  echo "<p class='text-muted'> You don't have any mail. </p>";
  return;
}
$i = 0;
...
function getConnect($uid){
  ...
  $query = "
    SELECT User.Name as contact
    FROM User INNER JOIN Mail 
    WHERE Mail.IdFrom={$_SESSION['IdUser']} AND User.Name in(SELECT Name FROM User WHERE IdUser=Mail.IdTo) OR 
          Mail.IdTo  ={$_SESSION['IdUser']} AND User.Name in(SELECT Name FROM User WHERE IdUser=Mail.IdFrom)
    GROUP BY User.Name;";
